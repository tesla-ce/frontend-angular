// import { Validators } from '@angular/forms';

export const CourseActivityConfig = {
  validator: null,
  fields: {
    name: {
      key: 'name',
      creable: true,
      showable: true,
      editable: false,
      dataType: 'string',
      label: 'ENTITIES.ACTIVITY.CODE',
      inputType: 'text',
      inputName: 'code-input-name',
      formControlName: 'code-form-control-name',
      // placeholder: '0.1.2',
      required: true,
    },
    description: {
      key: 'description',
      creable: true,
      showable: true,
      editable: false,
      dataType: 'string',
      label: 'ENTITIES.ACTIVITY.DESCRIPTION',
      inputType: 'text',
      inputName: 'description-input-name',
      formControlName: 'description-form-control-name',
      // placeholder: '0.1.2',
      required: true,
    },
    start: {
      creable: true,
      showable: true,
      editable: false,
      key: 'start',
      dataType: 'dateTime',
      withTime: false,
      label: 'ENTITIES.ACTIVITY.START',
      inputType: 'dateTime',
      inputName: 'start-input-name',
      formControlName: 'start-form-control-name',
      // placeholder: 'Doe',
    },
    end: {
      creable: true,
      showable: true,
      editable: false,
      key: 'end',
      dataType: 'dateTime',
      withTime: false,
      label: 'ENTITIES.ACTIVITY.END',
      inputType: 'dateTime',
      inputName: 'end-input-name',
      formControlName: 'end-form-control-name',
      // placeholder: 'Doe',
    },
  },
};
